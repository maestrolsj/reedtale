...
(function($, window, document, undefined) {
    ...
    var Portfolio = {
        init: function(options, elem) {
            ...
            self.$elem = $(elem);
            self.$thumnail = $(elem).find('.thumbnail2');
        ...
        getContentPos: function(clicked) {
            var self = this,
                thumbnails = self.$elem.find('.thumbnail2'),
                contentPos = null;
        ...
        showContent: function(thumbnail2) {
            var $href = thumbnail2.attr('href'),
                self = this,
                contentPos = self.getContentPos(thumbnail2);

            // Remove existing stuff first
            self.$elem.find('li a.thumbnail2 .active-arrow').remove();
            self.$elem.find('li.content').remove();
            // Add active arrow
            thumbnail2.append('<span class="active-arrow"></span>');

            // Add content
            var $portfolioContent = $($href);
            var html = '<li class="content">' + $portfolioContent.html() + '</li>';
